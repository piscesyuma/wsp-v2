...
export const TableCard = ({
  ...
  if (isSmallIconView) {
    return (
      <button
        className={cn(
          "h-[56px] w-full rounded-3 border-l-4 bg-white-60 p-3 text-black-100 hover:bg-gray-200 active:bg-gray-400",
    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className={cn("flex flex-row")}>
          ...
          <PersonIcon className="ml-1 h-5 w-5 text-black-40" />
        </div>
      </div>
      <div
        className={cn(
          "flex truncate whitespace-nowrap text-left text-black-60",
          fontCaptionNormal
        )}
      >
        {section_name || "-"}
      ...
      <div className="mt-auto flex flex-row items-end justify-between">
        <div className="flex flex-col items-start">
          <div className={cn("flex text-black", fontCaptionNormal)}>
            {status}
          </div>
          {status !== "Free" && (
        ...
        {status !== "Free" && (
          <div className="flex flex-row items-center">
            ...
            <LabProfileIcon className="ml-1 h-5 w-5 text-black-40" />
          </div>
        )}
      </div>
    </button>
